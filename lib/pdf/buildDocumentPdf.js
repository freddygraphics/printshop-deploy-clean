...
export async function buildDocumentPdf({
  ...
  if (!isQuote && Array.isArray(payments) && payments.length > 0) {
    ...
    for (const payment of payments) {
      ...
      const fee = safeNumber(payment?.processingFee);

      let methodText = cleanText(payment?.method || "Payment");
      ...
      if (method === "card") {
        ...
        methodText =
          fee > 0
            ? `Card Payment ($${formatMoney(paymentAmount)} + $${formatMoney(fee)} Fee)`
            : `Card Payment ($${formatMoney(paymentAmount)})`;
      } else if (method === "zelle") {
        methodText = "Zelle Payment";
      } else if (method === "cash") {
      ...
      const chargedAmount =
        method === "card"
          ? safeNumber(payment?.amount) + fee
          : safeNumber(payment?.amount);

      const amountText = `-$${formatMoney(chargedAmount)}`;
  ...
  const totalProcessingFee = Array.isArray(payments)
    ? payments.reduce(
        (sum, payment) => sum + safeNumber(payment?.processingFee),
        0,
      )
    : 0;
  ...
  if (!isQuote && totalProcessingFee > 0) {
    drawTotalRow("Processing Fee", totalProcessingFee);

    drawTotalRow("Total Charged", totalCharged, {
      font: bold,
    });
  }
  if (!isQuote && Array.isArray(payments) && payments.length > 0) {
