export function buildInvoiceHtml({
  ...
  const showPaymentsHistory = hasPayments && totalPaid > 0;
        ...
        .map((p) => {
          ...
          const paymentLabel =
            String(p.method).toLowerCase() === "card"
              ? `Card Payment (+$${fee.toFixed(2)} Fee)`
...
<tr>
  <td>${date}</td>
  <td>${paymentLabel}</td>
  <td class="right">−$${totalCharged.toFixed(2)}</td>
</tr>
`;
        })
        .join("")
...
html2pdf().from(document.querySelector('.invoice-page')).save();
}
</script>
</body>
</html>
